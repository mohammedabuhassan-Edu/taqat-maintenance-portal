...
const profileSchema = z.object({
  full_name: z
    .string()
    .trim()
    .min(2, "validation.minLength|2")
    .max(80, "validation.maxLength|80"),
...
export function ProfilePage() {
  ...
  const pw = useForm<PasswordValues>({
  ...
  const saveProfile = form.handleSubmit(async (values) => {
    ...
    if (values.preferred_lang !== i18n.resolvedLanguage)
      await i18n.changeLanguage(values.preferred_lang);
          ...
          <form onSubmit={saveProfile} className="space-y-4" noValidate>
            ...
            {me.role === "tenant" ? (
              <Field label={t("profile.fields.apartment")} htmlFor="apartment">
                <Input
                  id="apartment"
                  value={apartment.data?.unit_number ?? t("common.unassigned")}
                  disabled
                />
              </Field>
            ) : null}
            <Field
              ...
              htmlFor="full_name"
              error={tv(t, form.formState.errors.full_name?.message)}
            >
            ...
            <Field
              ...
              error={tv(t, form.formState.errors.phone?.message)}
            >
              <Input
                id="phone"
                type="tel"
          ...
          {me.is_demo ? (
            ...
            <form onSubmit={savePassword} className="space-y-4" noValidate>
              <Field
                ...
                error={tv(t, pw.formState.errors.password?.message)}
              >
                <Input
                  id="new_password"
                  type="password"
              ...
              <Field
                ...
                error={tv(t, pw.formState.errors.confirm?.message)}
              >
                <Input
                  id="confirm_password"
                  type="password"
